...
def mouseInWin():
	return Vector(pygame.mouse.get_pos()[0], pygame.mouse.get_pos()[1])
...
class Menu:
	...
	def getSuperPos(self):
		...
		return self.pos
	def getSuperMenu(self):
		if self.menu:
			return self.menu.getSuperMenu()
		return self
...
class MenuElement:
	...
	def renderSurf(self, text=None):
		...
		if text:
			self.text = text
		self.surf = Menu._font.render(self.text, True, WHITE)
	def evaluate(self, dic):
	...
	def draw(self):
		...
		pygame.draw.rect(Menu._win, color, (buttonPos, self.size))
		if self.surf:
			Menu._win.blit(self.surf, (buttonPos[0] + self.size[0]/2 - self.surf.get_width()/2, buttonPos[1] + self.size[1]/2 - self.surf.get_height()/2))
...
class MenuElementUpDown(MenuElementButton):
	...
	def draw(self):
		...
		pygame.draw.polygon(Menu._win, rightColor, [(buttonPos[0] + self.size[0] - arrowSize, buttonPos[1] + border), (buttonPos[0] + self.size[0] - border - 1, buttonPos[1] + border), (buttonPos[0] + self.size[0] - border - 1, buttonPos[1] + arrowSize)])
		pygame.draw.polygon(Menu._win, leftColor, [(buttonPos[0] + border ,buttonPos[1] + self.size[1] - arrowSize), (buttonPos[0] + border, buttonPos[1] + self.size[1] - border - 1), (buttonPos[0] + arrowSize, buttonPos[1] + self.size[1] - border - 1)])
		Menu._win.blit(self.surf, (buttonPos[0] + self.size[0]/2 - self.surf.get_width()/2, buttonPos[1] + self.size[1]/2 - self.surf.get_height()/2))
...
class MenuElementToggle(MenuElementButton):
	...
	def draw(self):
		...
		pygame.draw.rect(Menu._win, color, (buttonPos, self.size))
		if self.value:
			pygame.draw.rect(Menu._win, Menu._toggleColor, ((buttonPos[0] + self.border, buttonPos[1] + self.border), (self.size[0] - 2 * self.border, self.size[1] - 2 * self.border)))
		Menu._win.blit(self.surf, (buttonPos[0] + self.size[0]/2 - self.surf.get_width()/2, buttonPos[1] + self.size[1]/2 - self.surf.get_height()/2))
...
class MenuElementComboSwitch(MenuElementButton):
	...
	def setItems(self, strings):
		for string in strings:
			surf = Menu._font.render(string, True, WHITE)
	...
	def draw(self):
		...
		Menu._win.blit(surf, (buttonPos[0] + self.size[0]/2 - surf.get_width()/2, buttonPos[1] + self.size[1]/2 - surf.get_height()/2))
		arrowBorder = 3
		...
		pygame.draw.polygon(Menu._win, rightColor, [buttonPos + i for i in polygonRight])
		pygame.draw.polygon(Menu._win, leftColor, [buttonPos + i for i in polygonLeft])
...
class MenuElementImage(MenuElement):
	...
	def draw(self):
		buttonPos = self.getSuperPos() + self.pos
		Menu._win.blit(self.surf, buttonPos)
		pygame.draw.rect(Menu._win, Menu._buttonColor, (buttonPos, self.surf.get_size()), 2)
...
class MenuElementInput(MenuElementButton):
	...
	def initialize(self):
		...
		self.surf = None
		self.cursorSpeed = 30 // 4
		self.showCursor = False
		self.timer = 0
		self.cursor = Menu._font.render("|", True, (255,255,255))
	...
	def draw(self):
		...
		if self.mode == "editing" and self.showCursor:
			Menu._win.blit(self.cursor, (buttonPos[0] + self.size[0]/2 - self.surf.get_width()/2 + self.surf.get_width(), buttonPos[1] + self.size[1]/2 - self.surf.get_height()/2))
...
class MenuElementLoadBar(MenuElement):
	...
	def draw(self):
		buttonPos = self.getSuperPos() + self.pos
		pygame.draw.rect(Menu._win, Menu._textElementColor, (buttonPos, self.size))
		# calculate size
		size = Vector(self.size[0] * (self.value / self.maxValue), self.size[1])

		# draw bar left to right direction
		if self.direction == 1:
			pygame.draw.rect(Menu._win, Menu._textElementColor, (buttonPos, self.size), 2)
			pygame.draw.rect(Menu._win, self.color, (buttonPos + Vector(2,2), size - Vector(4,4)))
		# draw bar right to left direction
		else:
			pygame.draw.rect(Menu._win, Menu._textElementColor, (buttonPos + Vector(self.size[0] - size[0], 0), size), 2)
			pygame.draw.rect(Menu._win, self.color, (buttonPos + Vector(self.size[0] - size[0] + 2, 2), size - Vector(4,4)))
...
def menuHandleEvents(event, handleMenuEvents):
